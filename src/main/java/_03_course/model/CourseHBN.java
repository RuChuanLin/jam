...
public class CourseHBN implements CourseDAO {
	...
	public List<ReserveRecord> getBid(int courseId) {
		String hql = "from ReserveRecord where courseId = :courseId and status in (0,1)";
	...
	@Override
	public int acceptBid(byte status,int courseId,int studentId) {
		String hql = "update ReserveRecord set status=:status where courseId=:courseId and studentId=:studentId";
		Session session = HibernateUtil.getSessionFactory().getCurrentSession();
		Transaction tx = null;
		int updateNumber = 0;
		try {
